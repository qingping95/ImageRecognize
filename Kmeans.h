...
//public:
    KMEANS(int k);
    vector<T> getCent(int idx);
    void loadDataSet(char *filename);
    void loadDataSet(vector< vector<T> >& data);
    void randCent();
...
template<typename T>
vector<T> KMEANS<T>::getCent(int idx)
{
    return centroids[clusterAssment[idx].minIndex];
}
template<typename T>
...
//        print(outpath);//update the centroids
    }//while
...
template<typename T>
void KMEANS<T>:: loadDataSet(vector< vector<T> >& data)
{
    dataSet.clear();
    for(int i = 0; i < data.size(); i++)
    {
        ...
        {
            dt.push_back(data[i][j]);
        }
        dataSet.push_back(dt);
    }
